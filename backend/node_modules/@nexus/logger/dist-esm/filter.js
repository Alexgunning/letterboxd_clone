...
import { casesHandled, createContextualError, getLeft, rightOrThrow } from './utils';
 ...
 */
export function parse(defaults, pattern) {
 ...
 */
export function parseOne(criteriaDefaults, pattern) {
    ...
    if (path.value !== null) {
        ...
        if (invalidPathPartNames.length) {
            return left(createInvalidPattern(originalInput, `Path segment names must only contain ${String(validPathSegmentNameRegex)}.`));
        }
    }
 ...
 */
export function test(patterns, log) {
    let yaynay = null;
 ...
 */
export function parseUnsafe(defaults, pattern) {
...
export function renderSyntaxError(input) {
    ...
    if (!multipleInputs) {
        ...
        message = `Your log filter's pattern${foundIn} was invalid: "${chalk.red(pattern)}${hint}"\n\n${renderSyntaxManual()}`;
    }
    else if (!multipleErrors) {
        const e = getLeft(badOnes[0]);
        const pattern = e === null || e === void 0 ? void 0 : e.context.pattern;
        const hint = (e === null || e === void 0 ? void 0 : e.context.hint) ? `. ${e.context.hint}` : '';
        message = `One of the patterns in your log filter${foundIn} was invalid: "${chalk.red(pattern)}"${hint}\n\n${renderSyntaxManual()}`;
    }
    else {
...
export function processLogFilterInput(defaults, pattern, foundIn) {
