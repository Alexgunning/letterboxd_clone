 ...
 */
const invalidTsConfig = exceptionType('invalid_tsconfig', ({ diagnostics }) => 'Your tsconfig.json is invalid\n\n' + ts.formatDiagnosticsWithColorAndContext(diagnostics, diagnosticHost));
export async function readOrScaffoldTsconfig(input) {
    ...
    if (tscfgReadResult.error) {
        return left(exception('Unable to read your tsconifg.json\n\n' +
    ...
    if (tsconfigParsed.options.noEmit !== true) {
        ...
        log.warn(`Please set ${setting} to true. This will ensure you do not accidentally emit using ${chalk.yellowBright(`\`$ tsc\``)}. Use ${chalk.yellowBright(`\`$ nexus build\``)} to build your app and emit JavaScript.`);
    }
    /**
 ...
 */
export function tsconfigTemplate(input) {
