...
export const NEXUS_DEFAULT_RUNTIME_CONTEXT_TYPEGEN_PATH = fs.path('node_modules', '@types', 'typegen-nexus-context', 'index.d.ts');
/**
 * Run the pure extractor and then write results to a typegen module.
 */
export async function generateContextExtractionArtifacts(layout) {
    log.trace('starting context type extraction');
    const errProgram = createTSProgram(layout, { withCache: true });
    if (isLeft(errProgram))
        return errProgram;
 ...
 */
export async function writeContextTypeGenFile(contextTypes) {
