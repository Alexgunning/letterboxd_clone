...
import { createStartModuleContent, prepareStartModule, START_MODULE_NAME, } from '../../runtime/start/start-module';
...
import { computeBuildOutputFromTarget, logTargetPostBuildMessage, normalizeTarget, validateTarget, } from './deploy-target';
const log = rootLogger.child('build');
export async function buildNexusApp(settings) {
    var _a, _b, _c, _d, _e;
    process.env.NEXUS_BUILD = 'true';
    const startTime = Date.now();
    const deploymentTarget = normalizeTarget(settings.target);
    const buildOutput = (_b = (_a = settings.output) !== null && _a !== void 0 ? _a : computeBuildOutputFromTarget(deploymentTarget)) !== null && _b !== void 0 ? _b : undefined;
    ...
    if (!layout.tsConfig.content.options.noEmit) {
        await writeStartModule({
            layout: layout,
            startModule: prepareStartModule(tsBuilder, createStartModuleContent({
                internalStage: 'build',
                layout: layout,
 ...
 * nexus app.
 */
export async function writeStartModule({ startModule, layout, }) {
